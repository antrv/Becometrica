...
public enum WindowClassStyles: uint
{
    ...
    /// Because window classes are process specific, it is possible for multiple threads of
    /// an application to create a window of the same class. It is also possible for
    ...
    /// Typically, this is enabled for small, short-lived windows such as menus to emphasize
    /// their Z-order relationship to other windows. Windows created from a class with this style must be
    ...
    /// the child can draw on the parent. A window with the CS_PARENTDC style bit receives
    /// a regular device context from the system's cache of device contexts.
    ...
    /// This style increases the time required to display the window, because the system must first allocate
